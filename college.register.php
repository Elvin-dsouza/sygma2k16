					...
					function read_text($instr)
					{
						...
						else
						{
								...
								else// inserting values into the table when there are no errors found
								{
									...
									$qry= $dbhandle->prepare("INSERT INTO `college` (`college_name`,`email`,`code`) VALUES(?,?,?)");
									$qry->bind_param("sss",$dbarray['college_name'],$dbarray['contact_email'],$dbarray['college_code']);
									$_SESSION['college_registeration']=1;
									$_SESSION['college_name']=$dbarray['college_name'];
									$_SESSION['college_code']=$dbarray['college_code'];
									$_SESSION['college_id']=$dbhandle->insert_id;
									$id=$dbhandle->insert_id;
									$qry->execute();
									$qry->close();
									$dbhandle->close();
									header('../syg/dash.php?newreg=1&cid='.$id);
